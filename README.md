...
4 本指のタップで開くアプリ内デバッグメニュー。専用のオーバーレイと `Navigator` の中で完結するため、
アプリのルーターやアナリティクスに一切触れません。

- **Flutter 以外の依存はゼロ。**
- **メニュー項目は単なるウィジェット。** 実装すべきプラグインインターフェースはありません。`ListTile`、
  `ExpansionTile`、複数行の `Column` — `ListView` に入れられるものなら何でも使えます。
- **アプリのナビゲーションに漏れ出さない。** ウィンドウが専用の `Navigator` を持つので、デバッグ用の
  サブページはルートテーブルにも `NavigatorObserver` にも画面表示アナリティクスにも現れません。
- **アプリと入力を奪い合わない。** ポインタはジェスチャーアリーナに参加しない `Listener` で数え、
  キーボードショートカットは `HardwareKeyboard` から読むので、テキストフィールドのフォーカスを
  奪いません。
- **リリースでは既定で無効。** `enabled` を flavor で切り替えれば、tree shaking で全体が取り除かれます。
